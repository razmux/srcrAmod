...
enum {
	...
	MF_NOSTORAGE,
	MF_NOGUILDSTORAGE,
	MF_NODISCOUNT, // [ RagnaDev ]
	MF_GUILDMIN, // [ RagnaDev ]
};
 ...
 * null if not found
 * - [RagnaDev] Bug Fix (no muestra null)
 *------------------------------------------*/
...
BUILDIN_FUNC(successenchant)
{
	...
	// [RagnaDev] Guardamos en item_tmp antes de borrar el item
	memset(&item_tmp, 0, sizeof(item_tmp));
	...
	// [RagnaDev] Procedemos a asignar el enchant
...
BUILDIN_FUNC(getmapflag)
{
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_NODISCOUNT:			script_pushint(st,map[m].flag.nodiscount); break; // [ RagnaDev ]
			case MF_GUILDMIN:			script_pushint(st,map[m].flag.guild_min); break; // [ RagnaDev ]
		}
	}
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(setmapflag)
{
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_NODISCOUNT:			map[m].flag.nodiscount=1; break; // [RagnaDev]
			case MF_GUILDMIN:			map[m].flag.guild_min=1; break; // [RagnaDev]
		}
	}
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(removemapflag)
{
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_NODISCOUNT:			map[m].flag.nodiscount=0; break; // [RagnaDev]
			case MF_GUILDMIN:			map[m].flag.guild_min=0; break; // [RagnaDev]
		}
	}
	return SCRIPT_CMD_SUCCESS;
}
...
/*==========================================
 * Diego Soto Simple funcion mes que presenta 
...
/*==========================================
 * Diego Soto Simple funcion para saber si una
 ...
 * [RagnaDev] Diego Soto MVP vs MVP
 **/
